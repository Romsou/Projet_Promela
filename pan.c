/*** Generated by Spin Version 6.5.1 -- 7 March 2020 ***/
/*** From source: berger.pml ***/
...
iniglobals(int calling_pid)
{
	{	int l_in;
		...
		now.canal = addqueue(calling_pid, 1, 1);
#ifdef VAR_RANGES
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("p[l_in]", now.p[l_in]);
		}
	}
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.canal) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.canal) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.canal) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.canal) )))
			    ) return 0; break;
...
{	/* int i; */
	...
	printf("	mtype  aller:	2\n");
	printf("	mtype  retour:	1\n");
	printf("	chan canal (=%d):	len %d:\t", now.canal, q_len(now.canal));
	c_chandump(now.canal);
}
void
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("retour"); return;
	case 2: Printf("aller"); return;
